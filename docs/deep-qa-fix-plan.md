...
- 会话问答、课程资料、知识库、自动资源生成、思维导图、练习题、Markdown PPT、学习路径、错题本、学习报告、画像、资源中心都能点击使用。
...
- 固定《高数上》问题提问后自动返回五类资源：思维导图、练习题、讲义、学习路径、Markdown PPT。
- 前端交互检查：导图默认可读知识树，学习路径在预览区展示，错题答错后原地反馈。
- 资源中心下载 Markdown 文件包含教材依据、Verifier、生成来源和 fallback 状态。
...
- [x] 升级 demo 下载文件格式。
  - 讲义下载 Markdown/TXT。
  - 思维导图下载 Markdown，包含可读知识结构和 Mermaid 备份。
  - 练习题下载 Markdown，包含题目、选项、答案、解析和错因。
  - PPT 下载 Markdown 教学版，包含讲稿、板书、例题、自测和错因提醒。
  - 验收：资源中心下载文件可打开，且包含教材定位、Verifier 和生成来源。

### P2：答辩稳定性与项目观感

- [x] 将本轮深检命令固化成 `scripts/deep_qa_check.py`。
- [x] 增加 GitHub Actions：语法检查、前端 JS 检查、敏感文件检查。
- [x] 整理 README 中 “一键演示” 和当前真实按钮名称，避免写不存在的按钮。
- [x] 固化答辩课程：当前 Demo 默认提供“高等数学上册”课程上下文和《高数上.pdf》依据。
...
当前 P0 主链路已通过本地 smoke 和 deep QA。后续优先做答辩 PPT 正式文件、一次 Spark 真连接验证，以及答辩机器上的最终复跑。

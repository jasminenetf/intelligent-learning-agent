...
graph TB
    ...
    subgraph Data["数据层"]
        SQL[(SQLite/PostgreSQL<br/>课程/画像/会话/资源)]
        CHROMA[(ChromaDB<br/>向量索引)]
    end

    subgraph AI["AI 层"]
        SP[Spark API<br/>主LLM]
...
**说明**: 当前答辩 Demo 前端以免登录体验访问聚合 API。LobeChat 可选接入 /v1 兼容接口。资源生成链路：Profile+RAG → Spark/fallback → JSON → 渲染器。
...
flowchart LR
    ...
    PROMPT --> LLM[Spark 主引擎<br/>DeepSeek/Mock fallback]
    LLM --> PARSE_JSON[JSON 清洗+解析]
...
**说明**: LangGraph 多 Agent 工作流。supervisor 调度各子 Agent，profile_agent 维护画像，rag_agent 检索课程资料，resource_agent 调用 Spark/fallback 生成内容，verifier 校验答案与资料一致性并标注引用来源。
...
```mermaid
flowchart TD
    START([输入: topic + course_id + user]) --> PROF[get_or_create_profile<br/>6维画像]
    PROF --> RAG_RET[search_course<br/>RAG 检索 top_k=8]
    RAG_RET --> CHUNKS{有资料?}
    CHUNKS -- 无 --> ERR[返回错误]
    CHUNKS -- 有 --> BUILD_PROMPT[构建 Prompt<br/>topic + context + profile]
    BUILD_PROMPT --> CALL_LLM{Spark/fallback<br/>可用?}
    CALL_LLM -- 是 --> LLM_GEN[LLM 生成 JSON]
    CALL_LLM -- 否/Mock --> FALLBACK[fallback_template]
    LLM_GEN --> CLEAN[_clean_json<br/>去围栏+修复尾逗号]
...
**说明**: 统一生成管线。优先 Spark 真 LLM，失败时可切 DeepSeek 或 Mock/template。每个资源标记 generated_by 和 fallback_used，前端可据此展示生成来源。
